...
- 2026-07-11: On the exact historical `2000 x 2000` circle case, warmed PETSc/Hypre completed end-to-end in 4.24 s (7 iterations), closely reproducing the old 4.46 s result. GAMG completed in 7.95 s, PyAMG methods in 9.16–12.50 s, and direct solvers in 11.80–109.25 s.
...
- 2026-07-11: Audited the historical direct-solver table. Git history indicates it likely used the `2048 x 2048` self-affine rough-contact sweep rather than the rounded `2000 x 2000` circle assumption. Matched-case tests plus native threading recover Pardiso to 9.44 s versus 8.53 s historically; remaining CHOLMOD/MUMPS differences are consistent with the current reference-Netlib BLAS/PETSc build and unrecorded historical hardware.
